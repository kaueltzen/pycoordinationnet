...
get_model = lambda devices: GraphCoordinationNet(
    ...
    # Dense layer options
    layers = [1024, 512, 128, 64, 1], dropout = 0.0, skip_connections = False, batchnorm = False,
    # Data options
    batch_size = 64, val_size = 0.1,
    # Optimizer options
    optimizer = 'AdamW', weight_decay=0.05, scheduler = 'plateau', patience_sd = 5, patience_es = 50, lr = 1e-3, num_workers = 2,
    # Other
    default_root_dir = f'checkpoints', devices=devices)
...
def train_and_eval(inputfile, outputfile, devices):
    ...
    data_train, data_test = torch.utils.data.random_split(MPData(inputfile), [0.8, 0.2], generator=torch.Generator().manual_seed(42))
